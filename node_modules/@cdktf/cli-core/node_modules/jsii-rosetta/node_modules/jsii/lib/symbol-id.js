...
const types_versions_1 = require("./types-versions");
...
class Helper {
    ...
    assemblyRelativeSourceFile(sourceFileName, asm) {
        ...
        sourcePath = (0, types_versions_1.undoTypesVersionsRedirect)(sourcePath, packageInfo.typesVersions);
    ...
    findPackageInfo(from) {
        ...
        const packageJsonDir = (0, utils_1.findUp)(from, (dir) => fs.pathExistsSync(path.join(dir, 'package.json')));
        if (!packageJsonDir) {
        ...
        const { jsii, typesVersions } = fs.readJsonSync(path.join(packageJsonDir, 'package.json'));
        const result = {
            packageJsonDir,
            outdir: jsii?.outdir,
