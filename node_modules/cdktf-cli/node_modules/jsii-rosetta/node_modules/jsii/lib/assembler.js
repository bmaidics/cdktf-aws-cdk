...
class Assembler {
    ...
    emit() {
        ...
        if (sourceFile == null) {
            ...
            if (symbol) {
                ...
                for (const node of moduleExports) {
                    this._visitNode(node.declarations[0], new EmitContext([], this.projectInfo.stability));
                }
            }
        }
        ...
        if (!validationResult.emitSkipped) {
            const zipped = (0, spec_1.writeAssembly)(this.projectInfo.projectRoot, _fingerprint(assembly), { compress: this.compressAssembly ?? false });
    ...
    _getFQN(type, typeAnnotationNode, typeUse, isThisType) {
        ...
        if (!fqn || !this._dereference({ fqn }, sym.valueDeclaration)) {
            if (!hasError) {
                this._diagnostics.push(jsii_diagnostic_1.JsiiDiagnostic.JSII_3002_USE_OF_UNEXPORTED_FOREIGN_TYPE.create(typeAnnotationNode, fqn ?? tsName, typeUse, pkg).addRelatedInformation(typeDeclaration, `The referenced type is declared here`));
                hasError = true;
            }
        }
        return fqn;
    ...
    _visitNode(node, context) {
        ...
        if (ts.isExportSpecifier(node)) {
            ...
            return this._visitNode(resolvedSymbol.valueDeclaration ?? resolvedSymbol.declarations[0], context);
        }
        ...
        for (const submodule of this._submodules.keys()) {
            ...
            if (colliding != null) {
                ...
                this._diagnostics.push(jsii_diagnostic_1.JsiiDiagnostic.JSII_5011_SUBMODULE_NAME_CONFLICT.create(ts.getNameOfDeclaration(node) ?? node, submodule.name, jsiiType.name, candidates).addRelatedInformation(submoduleDeclName, `This is the conflicting submodule declaration`));
            }
        }
    ...
    _visitClass(type, ctx) {
        ...
        for (let base of type.getBaseTypes() ?? []) {
            ...
            // eslint-disable-next-line no-await-in-loop
            const ref = this._typeReference(base, type.symbol.valueDeclaration, 'base class');
    ...
    _visitDocumentation(sym, context) {
        ...
        if (!ts.isInterfaceDeclaration(decl) && result.hints.struct) {
            this._diagnostics.push(jsii_diagnostic_1.JsiiDiagnostic.JSII_7001_ILLEGAL_HINT.create(_findHint(decl, 'struct'), 'struct', 'interfaces with only readonly properties').addRelatedInformation(ts.getNameOfDeclaration(decl) ?? decl, 'The annotated declaration is here'));
        ...
        for (const declaringType of [type, ...erasedBases]) {
            for (const member of declaringType.getProperties()) {
                ...
                if (ts.isMethodDeclaration(member.valueDeclaration) ||
                    ...
                    // eslint-disable-next-line no-await-in-loop
                    this._visitMethod(member, jsiiType, ctx.replaceStability(jsiiType.docs?.stability), (type.symbol.valueDeclaration ??
                        type.symbol.declarations[0]));
                }
                    ...
                    // eslint-disable-next-line no-await-in-loop
                    this._visitProperty(member, jsiiType, ctx.replaceStability(jsiiType.docs?.stability), (type.symbol.valueDeclaration ??
                        type.symbol.declarations[0]));
                }
                else {
                    this._diagnostics.push(jsii_diagnostic_1.JsiiDiagnostic.JSII_9998_UNSUPPORTED_NODE.create(_nameOrDeclarationNode(member), (member.valueDeclaration ?? member.declarations[0]).kind));
                }
            }
        }
            ...
            if ((jsiiType.methods ?? []).length === 0) {
                ...
                this._diagnostics.push(jsiiType.methods.reduce((diag, mthod) => {
                    ...
                    return node
                        ...
                        : diag;
                }, jsii_diagnostic_1.JsiiDiagnostic.JSII_7001_ILLEGAL_HINT.create(_findHint(declaration, 'struct'), 'struct', 'interfaces with only readonly properties').addRelatedInformation(ts.getNameOfDeclaration(declaration) ?? declaration, 'The annotated declartion is here')));
            }
    ...
    _visitMethod(symbol, type, ctx, declaringTypeDecl) {
        ...
        if (type.name === Case.pascal(symbol.name)) {
            this._diagnostics.push(jsii_diagnostic_1.JsiiDiagnostic.JSII_5019_MEMBER_TYPE_NAME_CONFLICT.create(declaration.name, 'method', symbol, type).addRelatedInformation(declaringTypeDecl.name ?? declaringTypeDecl, `The declaring ${type.kind} is introduced here`));
        }
    ...
    _visitProperty(symbol, type, ctx, declaringTypeDecl) {
        ...
        if (type.name === Case.pascal(symbol.name)) {
            this._diagnostics.push(jsii_diagnostic_1.JsiiDiagnostic.JSII_5019_MEMBER_TYPE_NAME_CONFLICT.create(signature.name, 'property', symbol, type).addRelatedInformation(declaringTypeDecl.name ?? declaringTypeDecl, `The declaring ${type.kind} is introduced here`));
        }
        if (isProhibitedMemberName(symbol.name)) {
        ...
        const property = bindings.setPropertyRelatedNode({
            ...this._optionalValue(this._typeChecker.getTypeOfSymbolAtLocation(symbol, signature), signature.name, 'property type'),
            abstract: _isAbstract(symbol, type) || undefined,
            name: symbol.name,
            protected: _isProtected(symbol) || undefined,
            static: _isStatic(symbol) || undefined,
    ...
    findPackageInfo(fromDir) {
        ...
        return packageInfo;
        function _findPackageInfo(fromDir) {
...
function _isVoid(type) {
    if (_isPromise(type)) {
        const typeRef = type;
        return (typeRef.typeArguments != null &&
