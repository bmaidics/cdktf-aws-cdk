...
class DeprecatedRemover {
        ...
        for (const [fqn, typeInfo] of Object.entries(assembly.types)) {
            ...
            if ((0, spec_1.isEnumType)(typeInfo)) {
                ...
                typeInfo.members.forEach((mem) => {
                    if (mem.docs?.stability === spec_1.Stability.Deprecated &&
            ...
            if ((0, spec_1.isClassType)(typeInfo) &&
                typeInfo.base != null &&
                    ...
                    if (!strippedFqns.has(fqn)) {
                        ...
                        if (!originalSet.has(fqn)) {
                            this.transformations.push(Transformation.addInterface(this.typeChecker, bindings.getClassOrInterfaceRelatedNode(typeInfo), fqn in assembly.types
                                ? (bindings.getInterfaceRelatedNode(assembly.types[fqn]) ?? fqn)
                                : fqn));
                        }
                        continue;
                    }
            ...
            typeInfo.methods?.forEach((meth) => {
                if (meth.docs?.stability === spec_1.Stability.Deprecated &&
            ...
            typeInfo.properties?.forEach((prop) => {
                if (prop.docs?.stability === spec_1.Stability.Deprecated &&
    ...
    findLeftoverUseOfDeprecatedAPIs(assembly, strippedFqns) {
        ...
        for (const type of Object.values(assembly.types)) {
            ...
            type.methods?.forEach((method) => result.push(...this.verifyCallable(assembly, strippedFqns, method)));
            type.properties?.forEach((property) => result.push(...this.verifyProperty(assembly, strippedFqns, property)));
        }
        return result;
    }
    verifyCallable(assembly, strippedFqns, method) {
        const diagnostics = new Array();
        const deprecatedReturnFqn = (0, spec_1.isMethod)(method) &&
            method.returns &&
...
class Transformation {
    ...
    static removeBaseClass(typeChecker, node) {
        return new Transformation(typeChecker, node, (declaration) => {
            ...
            return {
                node: ts.updateClassDeclaration(declaration, declaration.decorators, declaration.modifiers, declaration.name, declaration.typeParameters, declaration.heritageClauses?.filter((clause) => clause.token !== ts.SyntaxKind.ExtendsKeyword), declaration.members),
            };
        });
    }
    ...
    static typeReference(type, context, typeChecker) {
        ...
        if (typeof type === 'string') {
            ...
            if (typeSource === contextSource) {
                ...
                const syntheticImportName = ts.createUniqueName((0, path_1.basename)(typeSource));
                syntheticImport = ts.createImportDeclaration(undefined /* decorators */, undefined /* modifiers */, ts.createImportClause(undefined, ts.createNamespaceImport(syntheticImportName)), ts.createStringLiteral(`./${(0, path_1.relative)((0, path_1.dirname)(contextSource), typeSource)}`));
    ...
    visitor(node) {
        ...
        if (ts.isExportDeclaration(node) && node.moduleSpecifier) {
            ...
            if (node.exportClause != null && moduleExports) {
                ...
                const filteredElements = bindings.elements?.filter((elt) => exportedNames.has(elt.name.text));
